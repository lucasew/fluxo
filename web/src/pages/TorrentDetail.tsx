  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <button
          onClick={() => navigate("/")}
          className="btn btn-ghost btn-sm gap-2 pl-0"
        >
        ...
        <div className="card-body">
          ...
          <div className="py-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span>
                {formatBytes(torrent.bytesCompleted)} /{" "}
                {formatBytes(torrent.bytesTotal)}
              </span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div className="stat p-2 bg-base-200 rounded-box">
              ...
              <div className="stat-value text-lg">
                {torrent.peers.total} ({torrent.peers.incoming} in /{" "}
                {torrent.peers.outgoing} out)
              </div>
            </div>
            <div className="stat p-2 bg-base-200 rounded-box">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs opacity-70">
            <div>
              <span className="font-bold">Hash:</span> {torrent.infoHash}
            </div>
            <div>
                ...
                {torrent.files.map((file, idx) => {
                  const fProgress =
                    Number(file.length) > 0
                      ? (Number(file.bytesCompleted) / Number(file.length)) *
                        100
                      : 0;
                  return (
                    <tr key={idx}>
